...
def load_train_hdf(path):
    return pd.concat([
        ...
        for i in (1, 2)], axis=0, ignore_index=True, copy=False)
...
def find_closest_hit_per_station(row):
    result = np.full(N_FOI_FEATURES, NA_VALUE, dtype=np.float32)
    ...
    closest_d0_per_station = result[24:28]
    closest_d1_per_station = result[28:32]
    closest_d2_per_station = result[32:36]
    
    center_mass_x_per_station = result[36:40]
    center_mass_y_per_station = result[40:44]
    ...
    for station in range(N_STATIONS):
        ...
        closest_d0_per_station[station] = distances_2.min()
        closest_d1_per_station[station] = distances_2.max()
        closest_d2_per_station[station] = distances_2.mean()
        
        center_mass_x_per_station[station] = row["Lextra_X[%i]" % station] - row["FOI_hits_X"][hits].mean()
        center_mass_y_per_station[station] = row["Lextra_Y[%i]" % station] - row["FOI_hits_Y"][hits].mean()
